...
/**
 * Search majors with sorting logic
 * POST /api/searchMajors
 */
router.post('/searchMajors', async (req: Request, res: Response) => {
  try {
    ...
    const db = getDb();
    const col = db.collection('majors');

    const query: any = {
      $or: [
        { chinese_name: new RegExp(keyword, 'i') },
        { english_name: new RegExp(keyword, 'i') }
      ]
    };
    
    if (level) {
      query.level = level; // e.g. 'Bachelor', 'Master'
    }

    const candidates = await col.find(query).limit(100).toArray();

    // Sorting logic (same as universities)
