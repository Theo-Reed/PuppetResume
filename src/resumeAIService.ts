...
1. **身份必须完全一致**：你生成的简历【职位名称】(\`position\`) 必须且只能是：“${job.title_chinese}”。严禁自作主张修改为“商业化运营”、“产品经理”等。
2. **推翻重写原则**：用户的原始背景（如程序员、运营等）如果与“${job.title_chinese}”不符，你必须【彻底抹除】原始职位的技术/运营痕迹。生成的简历必须让 HR 认为该用户一直就在从事“${job.title_chinese}”相关工作。
3. **经历强行适配**：
   - 保持公司（如 ${profile.workExperiences.map(e => e.company).join('、')}）和时间段不变。
   - 职位名重塑：将所有职位名改为与“${job.title_chinese}”强相关的称呼（如：招聘顾问、资深猎头、HRBP等）。
   - 职责描述重塑：必须使用“人才寻访”、“面试评估”、“简历筛选”、“猎头对接”等行业专业术语。严禁提到代码、产品运营或商业化。
