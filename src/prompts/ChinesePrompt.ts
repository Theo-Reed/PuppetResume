...
1. **职责描述 (Responsibilities)**：
   ...
   - **长度强制要求**：每条职责描述的视觉字数点数（中文字符计1点，英数/标点计0.5点）必须精准控制在 **${context.maxCharPerLine || 80} 到 ${Math.floor((context.maxCharPerLine || 80) * 1.1)}** 点之间。
   - **重要性排序**：必须按重要性从高到低排列。
   - **STAR法则 + 数据化**：必须包含具体数据/百分比/量级，突出"我"的主导作用。
   - **严禁空泛形容词**：禁止使用"大幅提升"、"显著改善"等无数据支撑的词汇。
2. **个人简介 (IMPORTANT)**：
   - 表现为 "${targetTitle}" 领域的资深专业人士。
   - **字数与格式**：总字数控制在 **120-150 字** 之间，必须分为 **两段**。
   - **内容倾向**：第一段侧重技术栈深度与行业广度，第二段侧重启发性成果、方法论或软技能。
   - **严禁使用带小数点的年限**：禁止写“拥有 5.8 年经历”，必须四舍五入为整数。
3. **技能列表 (Skills)**：
   - **结构强制要求**：**必须生成 4 组技能分类，且每组分类必须包含且仅包含 4 条技能点**。
   - **长度约束**：
     - **最低长度**：不得低于“熟悉 Docker/K8s 容器化技术”的长度。
     - **最高长度**：在最低长度的基础上，增加不超过 10 个英文字符的长度。
...
4. **排版 (Strict Quantity Control)**：
   ...
   - **标签数量限制**：全篇简历中（含简介和经历）<b> 标签总数 4-5 个，<u> 标签总数 3-4 个。
5. **内容填充密度控制**：
   - 请在生成时严格自我审查各条 `responsibilities` 的字数长度是否符合上述点数要求。
...
```json
{
  ...
  "personalIntroduction": "分两段，120-150字...",
  "professionalSkills": [
