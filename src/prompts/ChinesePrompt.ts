...
1. **身份定义（Resume Header）**：生成的简历【职位名称】(\`position\`) **必须是该行业的通用、自然且专业的职称**。
   ...
   - 必须把 "${targetTitle}" 清洗为一个标准的、简短的职业称谓。
   - **关键要求：**生成的职位名称必须符合中文语言习惯，读起来朗朗上口，**严禁出现“翻译腔”或生造词**。
     - ❌ 错误示例： "平台软件工程师" (听起来像机器翻译) -> ✅ 修正为： "后端开发工程师" 或 "软件开发工程师"（可以在个人简介中提及平台方向）。
     - ❌ 错误示例： "高级Java构建师" -> ✅ 修正为： "高级Java开发工程师" 或 "Java架构师"。
   - 规则：去掉所有后缀、连字符、括号、部门代码、招聘用语（如“急招”、“驻场”等）。
   - 错误示例： "业务开发代表 - 生态系统专家"
   - 正确示例： "资深业务开发代表" 或 "生态合作经理"
   - 让这个职称看起来像是一个真实的、资深的职业身份，而不是招聘广告的标题。
...
3. **经历强力重塑**：
   ...
   - **职位名称生成规则（非常重要）**：
     * **核心原则**：过往经历的职位名称必须是**该行业通用的标准职称**，**严禁直接照抄目标岗位名称**（除非目标岗位就是很通用的"产品经理"、"销售经理"等）。
     * **避免过长**：职位名称应简练，不要包含横线后的细分领域（如目标是"业务开发代表 - 生态系统专家"，过往经历应写"业务开发代表"或"客户经理"，而不是"业务开发代表 - 生态系统专家"）。
     * **层级递进**：早期经历使用初级/中级职称，近期经历使用高级/资深职称，体现职业成长路径。
...
- 职位名称: 根据目标岗位"${targetTitle}"和岗位描述灵活生成：
  * **严禁照抄目标岗位名称**：必须使用**该行业通用的标准职称**（如"产品经理"、"销售经理"、"Java工程师"）。
  * 避免复制长尾词：如果目标岗位有后缀（如" - 生态系统专家"），必须去掉后缀，只保留核心职位词。
  * 根据岗位描述生成最贴切的通用职称。
...
4. ****严禁照抄目标岗位名称**，必须使用行业的**通用标准职称**。
   * 如果目标岗位名称中包含连字符或修饰词（如" - "、"专家"），请提取核心职位词作为过往经历的名称。
...
7. 排版: 3-4 处 <b> 加粗，3-4 处 <u> 下划线。**严禁使用Markdown格式的加粗符号（如 ** ）**，必须使用HTML标签 <b> 来表示加粗。
